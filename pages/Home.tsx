...
import { Sparkles, Moon, CloudMoon } from 'lucide-react'; // Adicionei CloudMoon
...
const Home: React.FC<HomeProps> = ({ user }) => {
  ...
  const [hasDailyCard, setHasDailyCard] = useState(false); // Estado para saber se já tirou

  // Verifica se já tirou a carta do dia ao carregar
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full max-w-4xl mx-auto py-12 animate-fade-in-up">
      ...
      {/* --- NOVO: AVISO DA CARTA DO DIA (Só aparece se logado e não tirou) --- */}
      {user && !hasDailyCard && (
        <button 
