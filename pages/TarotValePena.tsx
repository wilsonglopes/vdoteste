...
const LOCAL_KEY = 'vozes_tarot_vale_pena_state';
const MAX_CARDS = 7; 
...
const TarotValePena: React.FC = () => {
  ...
  const TITLE = "Ele(a) Vale a Pena?";
  const POSITIONS = [
    "Ele(a) - Quem é", 
    "Como ele(a) me vê", 
    "O que sente sobre a relação", 
    "Oculto (O que não estou vendo)", 
    "Base da relação", 
    "Futuro da relação", 
    "Aprendizado/Conselho"
  ];
  ...
  const startReveal = async () => {
    ...
    try {
      ...
      contextPrompt += `\nInstrução: Faça uma análise crítica sobre compatibilidade, alertas ocultos e se o investimento emocional vale o retorno.`;
  ...
  const renderLayout = () => {
    const renderSlot = (index: number) => {
        ...
        return (
          <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="relative w-20 h-32 md:w-24 md:h-40 group">
            <div className="absolute -top-10 left-1/2 -translate-x-1/2 w-[160%] text-center z-20 pointer-events-none">
    ...
    return (
      <div className="flex flex-col items-center gap-8 mt-4">
         <div className="flex gap-4 md:gap-6">{renderSlot(0)}{renderSlot(1)}{renderSlot(2)}</div>
         <div className="flex gap-4 md:gap-6">{renderSlot(3)}{renderSlot(4)}{renderSlot(5)}</div>
         <div className="flex gap-4 md:gap-6">{renderSlot(6)}</div>
      </div>
    );
  };
    ...
    <div className="max-w-5xl mx-auto py-6 px-4 w-full flex flex-col items-center min-h-screen">
      ...
      {(step === 'reveal' || step === 'result') && (
        <div className="w-full flex flex-col items-center">
            <h2 className="text-2xl font-serif text-white mb-4 text-center">{step === 'reveal' ? 'A Verdade na Mesa' : 'Interpretação'}</h2>
