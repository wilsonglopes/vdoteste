...
const TarotMensal: React.FC = () => {
  ...
  const TITLE = "Método Mensal";
  const POSITIONS = ["Energia do Mês", "A Favor (Positivo)", "Contra (Negativo)", "Financeiro", "Amor/Pessoal", "Conselho"];
  ...
  const startReveal = async () => {
    ...
    try {
      ...
      contextPrompt += `\nInstrução: Foque em previsão temporal para os próximos 30 dias.`;
    ...
    <div className="max-w-5xl mx-auto py-6 px-4 w-full flex flex-col items-center min-h-screen">
      ...
      {(step === 'reveal' || step === 'result') && (
        <div className="w-full flex flex-col items-center">
            <h2 className="text-2xl font-serif text-white mb-4 text-center">{step === 'reveal' ? 'O Mês à Frente' : 'Previsão'}</h2>
            <div className="w-full overflow-x-auto py-8 flex justify-center bg-white/5 rounded-3xl border border-white/5 mb-8 min-h-[500px]">
                {renderLayout()}
            </div>
