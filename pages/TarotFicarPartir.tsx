...
const TarotFicarPartir: React.FC = () => {
  ...
  const TITLE = "Ficar ou Partir?";
  const POSITIONS = ["Situação Atual", "Por que ficar?", "Por que partir?", "Como me sentirei (Ficar)", "Como me sentirei (Partir)", "Conselho Final"];
  ...
  const startReveal = async () => {
    ...
    try {
      ...
      contextPrompt += `\nInstrução: Compare as duas opções (ficar vs partir). Analise prós, contras e sentimentos. Dê um veredito claro.`;
  ...
  const renderLayout = () => {
    ...
    return (
      <div className="flex justify-center gap-8 md:gap-16 mt-4 max-w-4xl mx-auto">
         {/* Lado Ficar */}
         <div className="flex flex-col gap-6 items-end border-r border-white/10 pr-6">
            <h4 className="text-xs font-bold text-green-400 uppercase tracking-widest border-b border-green-500/30 pb-1">Opção: Ficar</h4>
            <div className="mr-8">{renderSlot(0)}</div>
            <div className="flex gap-4">{renderSlot(1)}{renderSlot(3)}</div>
         </div>
         {/* Lado Partir */}
         <div className="flex flex-col gap-6 items-start pl-2">
            <h4 className="text-xs font-bold text-red-400 uppercase tracking-widest border-b border-red-500/30 pb-1">Opção: Partir</h4>
            <div className="flex gap-4">{renderSlot(2)}{renderSlot(4)}</div>
            <div className="ml-8 mt-4">{renderSlot(5)}</div>
         </div>
      </div>
    );
  };
    ...
    <div className="max-w-5xl mx-auto py-6 px-4 w-full flex flex-col items-center min-h-screen">
      ...
      {(step === 'reveal' || step === 'result') && (
        <div className="w-full flex flex-col items-center">
            <h2 className="text-2xl font-serif text-white mb-4 text-center">{step === 'reveal' ? 'O Dilema' : 'Veredito'}</h2>
            <div className="w-full overflow-x-auto py-12 flex justify-center bg-white/5 rounded-3xl border border-white/5 mb-8 min-h-[500px]">
                {renderLayout()}
            </div>
