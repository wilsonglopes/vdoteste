...
const MAX_CARDS = 5; // FIXO PARA ESTE JOGO
...
const TarotEx: React.FC = () => {
  ...
  // --- POSIÇÕES ESPECÍFICAS DA TIRADA DO EX ---
  const POSITIONS = [
    "O que sente por mim?",
  ...
  const handleCardSelect = (card: CardData) => {
    ...
    if (newSelection.length === MAX_CARDS) {
        setTimeout(() => {
            setRevealedLocal(0);
            setStep('reveal');
        }, 1000);
  ...
  const startReveal = async () => {
    ...
    try {
      ...
      // Prompt Específico para EX
      let contextPrompt = `\n\n[MÉTODO: TIRADA DO EX - 5 CARTAS]\n`;
      selectedCards.forEach((card, idx) => {
          contextPrompt += `- Posição ${idx + 1} (${POSITIONS[idx]}): Carta "${card.name}"\n`;
      });
      contextPrompt += `\nInstrução: Foque na dinâmica de ex-relacionamento, sentimentos remanescentes e possibilidade de reconciliação.`;
    ...
    const renderSlot = (index: number) => {
        ...
        return (
          <motion.div 
            ...
            className="relative w-20 h-32 md:w-24 md:h-40 group"
          >
            {/* Label */}
            <div className="absolute -top-8 left-1/2 -translate-x-1/2 w-[150%] text-center z-20">
                <span className="text-[9px] font-bold uppercase bg-black/80 text-purple-200 px-2 py-1 rounded border border-white/10 block leading-tight">
                  {label}
                </span>
            </div>
            {/* Carta */}
            <div className="w-full h-full relative">
                {!isRevealed ? (
                    <div className="absolute inset-0 bg-indigo-950 border-2 border-purple-500/50 rounded-lg flex items-center justify-center">
            ...
            {/* Número */}
            <div className="absolute -bottom-2 -right-2 bg-purple-600 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full font-bold border border-black">
                {index + 1}
            </div>
          </motion.div>
        );
  ...
  return (
    <div className="max-w-5xl mx-auto py-6 px-4 w-full flex flex-col items-center min-h-screen">
      ...
      <div className="w-full flex justify-between items-center mb-4">
         <button onClick={() => navigate('/nova-leitura')} className="flex items-center text-slate-400 hover:text-white gap-2 text-sm font-bold">
            <ArrowLeft size={18} /> Voltar
         </button>
         <div className="text-right">
            <h2 className="text-purple-300 font-serif text-lg">Tirada do Ex</h2>
            <span className="text-[10px] text-slate-500 uppercase tracking-widest">5 Cartas</span>
         </div>
      </div>

      {step === 'question' && (
        <QuestionStep question={question} setQuestion={setQuestion} onNext={() => setStep('selection')} onBack={() => navigate('/nova-leitura')} />
      )}
      ...
      {(step === 'reveal' || step === 'result') && (
        <div className="w-full flex flex-col items-center">
            <h2 className="text-2xl font-serif text-white mb-4 text-center">
                {step === 'reveal' ? 'Revelando a Mesa...' : 'Interpretação'}
            </h2>

            {/* MESA FIXA DE 5 CARTAS */}
            ...
            {step === 'reveal' && revealedLocal === MAX_CARDS && !reading && (
               <button onClick={startReveal} disabled={isLoadingAI} className="px-10 py-4 bg-green-600 hover:bg-green-500 text-white rounded-full font-bold flex items-center gap-3 animate-bounce">
            ...
            {step === 'result' && reading && (
                <div className="w-full max-w-3xl bg-slate-900/90 border border-purple-500/30 rounded-3xl p-8">
                  ...
                  <div className="mt-6 p-4 bg-purple-900/30 rounded-xl border border-purple-500/20">
                    <h4 className="text-purple-300 font-bold text-sm uppercase">Conselho</h4>
                    <p className="text-white italic">"{reading.summary}"</p>
                  </div>
