...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        // Validação básica para garantir que o WhatsApp foi preenchido
        if (!formData.whatsapp) {
          throw new Error("O WhatsApp é obrigatório para contato.");
        }
        ...
        const { error } = await supabase.auth.signUp({
          ...
          options: {
            data: {
              ...
              whatsapp: formData.whatsapp // Enviando para o Supabase
            }
          }
        });
        ...
        <div className="p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-sm">
              {isSignUp 
                ? "Cadastre-se para acessar os mistérios. Dados corretos são essenciais para uma interpretação precisa." 
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {isSignUp && (
              <div className="space-y-4 animate-fade-in">
                ...
                {/* CAMPO WHATSAPP NOVO - Estilo idêntico aos outros */}
                <div className="group">
                  <label className="block text-xs uppercase tracking-wider text-slate-500 mb-1 group-focus-within:text-purple-400 transition-colors">WhatsApp</label>
                  <input
                    type="tel"
                    required
                    className="w-full bg-slate-950/50 border border-white/10 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50 transition-all"
