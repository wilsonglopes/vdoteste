...
// Removi o CARDS_TO_SELECT daqui pois usaremos o 'limit' passado via prop
...
interface SelectionStepExProps {
  ...
  onBack: () => void;
  isMobile: boolean;
  limit: number; // ADICIONADO: Recebe o limite (ex: 5)
}
...
const SelectionStepEx: React.FC<SelectionStepExProps> = ({
  ...
  onBack,
  isMobile,
  limit // Usado para definir quantos slots aparecem
}) => {
  ...
  return (
    <div className="flex flex-col items-center w-full relative justify-start pt-4 md:pt-8 min-h-[80vh]">
      ...
      <div className="text-center z-20 px-4 mb-6">
        {/* Alterado para usar a variável limit */}
      ...
      {/* LEQUE DE CARTAS (CÓDIGO ORIGINAL MANTIDO 100% IGUAL) */}
      ...
      <div className="w-full max-w-5xl mx-auto z-30 px-4 mb-[4px]">
        <div className="bg-slate-950/80 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl">
          ...
          <div className={`flex justify-center gap-2 md:gap-4 mb-4 w-full ${isMobile ? 'flex-wrap' : 'overflow-x-auto no-scrollbar'}`}>
            ...
            {/* Alterado CARDS_TO_SELECT para limit aqui */}
          ...
          <div className="flex justify-end items-center px-2">
            <div className="flex items-center gap-4">
              {/* Alterado CARDS_TO_SELECT para limit aqui */}
              <span className="text-gold font-playfair text-lg">{selectedCards.length} / {limit}</span>
              
              {/* Alterado condição para limit */}
