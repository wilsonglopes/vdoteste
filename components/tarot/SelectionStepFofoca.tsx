...
const SelectionStepFofoca: React.FC<SelectionStepFofocaProps> = ({
  ...
  return (
    <div className="flex flex-col items-center w-full relative justify-start pt-4 md:pt-8 min-h-[80vh]">
      ...
      {/* LEQUE (Mantido igual para consistência) */}
      ...
      {/* SLOTS (Permite rolagem horizontal se não couber tudo) */}
      <div className="w-full max-w-5xl mx-auto z-30 px-4 mb-[4px]">
        <div className="bg-slate-950/80 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl">
          ...
          <div className={`flex justify-start md:justify-center gap-2 md:gap-3 mb-4 w-full overflow-x-auto pb-2 scrollbar-hide`}>
            {selectedCards.map((card, i) => (
              <motion.div key={card.id} initial={{ scale: 0, y: 20 }} animate={{ scale: 1, y: 0 }} className="relative flex-shrink-0">
                <img src={CARD_BACK_URL} className="w-10 h-16 md:w-12 md:h-20 rounded border border-pink-500 shadow-md" />
                <span className="absolute -top-2 -right-2 bg-pink-500 text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{i + 1}</span>
              </motion.div>
            ))}
            
            {Array.from({ length: limit - selectedCards.length }).map((_, i) => (
              <div key={i} className="w-10 h-16 md:w-12 md:h-20 rounded border border-dashed border-white/20 bg-white/5 flex-shrink-0" />
            ))}
          </div>

          <div className="flex justify-end items-center px-2">
            <div className="flex items-center gap-4">
              <span className="text-pink-400 font-playfair text-lg">{selectedCards.length} / {limit}</span>
              ...
              {selectedCards.length === limit && (
                <motion.button
                  ...
                  onClick={onNext}
                  className="bg-pink-600 text-white font-bold px-6 py-2 rounded-full shadow-lg hover:bg-pink-500 hover:scale-105 flex items-center gap-2 transition-all duration-200"
                >
