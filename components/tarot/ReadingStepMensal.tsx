...
interface ReadingStepMensalProps {
  question: string;
...
const ReadingStepMensal: React.FC<ReadingStepMensalProps> = ({
  question,
  selectedCards,
  revealedLocal,
  reading,
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const filename = user?.user_metadata?.name ? `Leitura_Mensal_${user.user_metadata.name}.pdf` : 'Leitura_Mensal.pdf';
      pdf.save(filename);
    } catch (error) {
  ...
  // --- LAYOUT MÉTODO MENSAL (6 CARTAS) ---
  const getCardStyle = (index: number) => {
    switch (index) {
      case 0: return "col-start-2 row-start-2 z-10 scale-105 shadow-2xl"; // 1: Energia do Mês (Centro)
      case 1: return "col-start-1 row-start-2"; // 2: Perspectiva Positiva (Esq)
      case 2: return "col-start-3 row-start-2"; // 3: Perspectiva Negativa (Dir)
      case 3: return "col-start-1 row-start-1"; // 4: Financeiro (Topo Esq)
      case 4: return "col-start-3 row-start-1"; // 5: Escolha (Topo Dir)
      case 5: return "col-start-2 row-start-3"; // 6: Conselho (Baixo)
      default: return "";
    }
  };
    ...
    <div className="w-full p-4 md:p-8 bg-transparent flex flex-col items-center">
      ...
      <div id="reading-result" ref={resultRef} className="w-full flex flex-col items-center">
        ...
        <div className="text-center mb-6 w-full">
          <h2 className="font-playfair text-3xl md:text-4xl text-white mb-2">Previsão Mensal</h2>
          <p className="text-slate-400 italic text-lg md:text-xl">"{question}"</p>
        </div>

        {/* GRID ESPECÍFICO 6 CARTAS (Mantido) */}
        <div className="grid grid-cols-3 gap-x-3 gap-y-4 max-w-lg mx-auto mb-12 w-full relative perspective-1000 py-4 justify-items-center">
        ...
        {revealedLocal === totalCards && (
          <div className="w-full animate-fade-in-up space-y-8 pb-8">
            {isLoadingAI || !reading ? (
              <div className="bg-slate-900/50 border border-cyan-500/20 p-10 rounded-2xl text-center backdrop-blur-md">
                <Loader2 className="w-12 h-12 text-gold animate-spin mx-auto mb-4" />
                <p className="text-purple-200 text-xl font-playfair">Calculando as tendências do mês...</p>
              </div>
            ) : (
              <>
                {/* --- LÓGICA DE ERRO --- */}
                {isError ? (
                    <div className="bg-red-900/40 border-l-4 border-red-500 p-8 rounded-r-xl backdrop-blur-md shadow-2xl text-center max-w-2xl mx-auto">
                        ...
                        <button 
                            onClick={onReveal} // Chama o reload da IA
                    ...
                    // --- RESULTADO NORMAL ---
                    <>
                        <div className="bg-slate-900/80 border-l-4 border-cyan-500 p-6 rounded-r-xl backdrop-blur-md">
                        <p className="text-lg text-purple-100 italic font-light leading-relaxed">{reading.intro}</p>
                        </div>

                        <div>
                        <h3 className="text-2xl text-white font-playfair mb-6 flex items-center gap-2"><Clock className="text-cyan-400" /> Detalhes do Mês</h3>
                        <div className="space-y-4">
                            {reading.individual_cards?.map((item: any, idx: number) => (
                            <div key={idx} className="bg-white/5 p-4 rounded-xl border border-white/5 hover:border-cyan-500/30 transition-colors">
                        ...
                        <div>
                            <h4 className="text-gold font-playfair text-2xl mb-2">Resumo do Mês</h4>
                            <p className="text-white text-lg font-medium">{reading.summary}</p>
                        </div>
                        <hr className="border-white/10" />
                        <div>
                            <p className="text-sm text-slate-500 uppercase tracking-widest mb-2">Conselho Principal</p>
      ...
      {revealedLocal === totalCards && reading && (
        <div className="w-full max-w-3xl flex flex-col md:flex-row gap-4 justify-center pb-20 mt-4">
          {/* Se houver erro, exibe o botão Tentar Novamente também no rodapé */}
          {isError ? (
             <button 
                onClick={onReveal} 
             ...
             // Se sucesso, exibe botões normais
             <>
...
export default ReadingStepMensal;
